...
from dlshogi.feature import (FEATURES_NUM, make_move_label, make_result,
                             set_board_features)
...
class HcpeDataLoader:
    ...
    def mini_batch(self, hcpe_vec):
        self.features.fill(0)
        for i, hcpe in enumerate(hcpe_vec):
            self.board.set_hcp(hcpe["hcp"])
            set_board_features(self.board, self.features[i])
    ...
    def pre_fetch(self):
        # sequentialに取ってくる
        hcpe_vec = self.data[self.i : self.i + self.batch_size]
        self.i += self.batch_size
        if len(hcpe_vec) < self.batch_size:
            return

        # メンバー関数をこの部分に適用して実行
        self.f = self.executor.submit(self.mini_batch, hcpe_vec)
